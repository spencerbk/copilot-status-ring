...
The simplest way to set the value is the `setup-status-ring` wizard, which prompts for 24 / 16 / 12 and both writes the choice into `.copilot-command-ring.local.json` *and* templates `NUM_PIXELS` directly into the copied `main.py` so the boot wipe matches before the first host message arrives. You can also set `pixel_count` directly in that file or `COPILOT_RING_PIXEL_COUNT` in the environment.

`NUM_PIXELS = 24` at the top of `main.py` is the *startup* default used only for the boot wipe before the first host message arrives. The setup wizard rewrites this when it copies firmware; edit it manually only if you are flashing without the wizard.
