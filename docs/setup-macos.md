...
3. Flash the **CircuitPython** UF2 if the board is not already running it.
4. Copy the prepared firmware files to `CIRCUITPY` if the installer did not copy them for you.
...
```bash
./install.sh --yes
./install.sh --firmware-target /Volumes/CIRCUITPY
...
5. Copy the firmware files:
   ...
   cp ~/.local/share/copilot-command-ring/firmware/circuitpython/boot.py /Volumes/CIRCUITPY/boot.py
   cp ~/.local/share/copilot-command-ring/firmware/circuitpython/code.py /Volumes/CIRCUITPY/code.py
   ```

6. Install `neopixel.mpy` from the [Adafruit CircuitPython Bundle](https://circuitpython.org/libraries) into `CIRCUITPY/lib/` if the installer did not already install it with `circup`.
...
- **Easiest:** rerun `setup-status-ring` and pick 24, 16, or 12 at the *Which ring size do you have?* prompt. The wizard writes `pixel_count` into `.copilot-command-ring.local.json` *and* rewrites the static `NUM_PIXELS` / `#define PIXEL_COUNT` in the copied firmware so the boot wipe matches.
- **Quick change:** run `copilot-command-ring set-pixels <count>` (e.g. `set-pixels 16`) to update `pixel_count` in your local config without rerunning the wizard. Other config fields are preserved, and the ring picks up the new count on the next Copilot CLI event.
...
- **Clean startup wipe:** the firmware-default `NUM_PIXELS` (CircuitPython, MicroPython) and `PIXEL_COUNT` (Arduino) is `24`. The wizard rewrites that constant for you when you flash through it; if you flash without the wizard, edit `NUM_PIXELS` / `#define PIXEL_COUNT` in the firmware to match your ring before uploading.
